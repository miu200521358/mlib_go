...
- コメントは日本語で記述する
- 全関数にドキュメントコメントを必須とする（テスト関数も含む）
- 複雑なロジックには説明コメントを追加する
- 日本語コメントを入れる前に、対象 Go ファイルへ `// 指示: miu200521358` を入れて非ASCII化する
- 新規 Go ファイル追加後は `internal/scripts/add_instruction_header.ps1` を先に実行する
