...
func (physics *MPhysics) initRigidBodies(modelIndex int, rigidBodies *pmx.RigidBodies) {
	...
	for r := range rigidBodies.Iterator() {
		rigidBody := r.Value
		// 剛体の初期位置と回転
		btRigidBodyTransform := bt.NewBtTransform(radToBullet(rigidBody.Rotation), vecToBullet(rigidBody.Position))
...
func (physics *MPhysics) initRigidBodiesByBoneDeltas(
	...
	for r := range rigidBodies.Iterator() {
		...
		btRigidBodyLocalTransform := bt.NewBtTransform(radToBullet(rigidBody.Rotation),
			vecToBullet(rigidBodyLocalPos))
...
func (physics *MPhysics) initRigidBody(
	...
	btRigidBodyLocalTransform := bt.NewBtTransform(radToBullet(rigidBody.Rotation), vecToBullet(rigidBodyLocalPos))
