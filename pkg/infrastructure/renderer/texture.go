...
func (rm *RenderModel) initTexturesGl(windowIndex int, textures *pmx.Textures, modelPath string) error {
	rm.textures = make([]*textureGl, textures.Length())

	for v := range textures.Iterator() {
		texture := v.Value
		texGl, err := rm.initTextureGl(windowIndex, texture, modelPath)
		if err != nil {
			mlog.D(fmt.Sprintf("texture error: %s", err))
			continue
		}
		rm.textures[texture.Index()] = texGl
	}

	return nil
}

func (rm *RenderModel) initTextureGl(
...
func (rm *RenderModel) initToonTexturesGl(windowIndex int) error {
	rm.toonTextures = make([]*textureGl, 10)
