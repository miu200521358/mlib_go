...
func decompressMSZipXFile(
	...
	for srcOffset < compressedBufferLen && uncompressedSum < finalSize {
		...
		if ret, err := DecompressRFC1951Block(blockData[2:], newBuffer[destOffset:destOffset+int(blockUncompressedSize)]); err != nil {
			return fmt.Errorf("DecompressRFC1951Block failed on block #%d: blockUncompressedSize=%d, blockCompressedSize=%d, ret=%d, err=%v",
				blockIndex, blockUncompressedSize, blockCompressedSize, ret, err)
		}
...
func DecompressRFC1951Block(compressed []byte, decompressed []byte) (int, error) {
	...
	return int(ctx.BytesDecompressed), nil
}
