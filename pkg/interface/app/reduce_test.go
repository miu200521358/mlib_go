...
func TestReduce_Gimme(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%s [%04.0f]", tt.name, tt.frame), func(t *testing.T) {
			reduceBf := reduceMotion.BoneFrames.Get(tt.name).Get(tt.frame)
			...
			if !bf.Position.NearEquals(reduceBf.Position, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Position want %v, expect %v",
					tt.name, tt.frame, bf.Position, reduceBf.Position)
			}
			if !bf.Rotation.NearEquals(reduceBf.Rotation, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Rotation want %v, expect %v",
					tt.name, tt.frame, bf.Rotation, reduceBf.Rotation)
			}
			if !bf.Curves.TranslateX.Start.NearEquals(&reduceBf.Curves.TranslateX.Start, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.TranslateX.Start want %v, expect %v",
					tt.name, tt.frame, bf.Curves.TranslateX.Start, reduceBf.Curves.TranslateX.Start)
			}
			if !bf.Curves.TranslateX.End.NearEquals(&reduceBf.Curves.TranslateX.End, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.TranslateX.End want %v, expect %v",
					tt.name, tt.frame, bf.Curves.TranslateX.End, reduceBf.Curves.TranslateX.End)
			}
			if !bf.Curves.TranslateY.Start.NearEquals(&reduceBf.Curves.TranslateY.Start, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.TranslateY.Start want %v, expect %v",
					tt.name, tt.frame, bf.Curves.TranslateY.Start, reduceBf.Curves.TranslateY.Start)
			}
			if !bf.Curves.TranslateY.End.NearEquals(&reduceBf.Curves.TranslateY.End, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.TranslateY.End want %v, expect %v",
					tt.name, tt.frame, bf.Curves.TranslateY.End, reduceBf.Curves.TranslateY.End)
			}
			if !bf.Curves.TranslateZ.Start.NearEquals(&reduceBf.Curves.TranslateZ.Start, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.TranslateZ.Start want %v, expect %v",
					tt.name, tt.frame, bf.Curves.TranslateZ.Start, reduceBf.Curves.TranslateZ.Start)
			}
			if !bf.Curves.TranslateZ.End.NearEquals(&reduceBf.Curves.TranslateZ.End, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.TranslateZ.End want %v, expect %v",
					tt.name, tt.frame, bf.Curves.TranslateZ.End, reduceBf.Curves.TranslateZ.End)
			}
			if !bf.Curves.Rotate.Start.NearEquals(&reduceBf.Curves.Rotate.Start, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.Rotate.Start want %v, expect %v",
					tt.name, tt.frame, bf.Curves.Rotate.Start, reduceBf.Curves.Rotate.Start)
			}
			if !bf.Curves.Rotate.End.NearEquals(&reduceBf.Curves.Rotate.End, 1e-6) {
				t.Errorf("%s [%04.0f]: BoneFrame.Curves.Rotate.End want %v, expect %v",
					tt.name, tt.frame, bf.Curves.Rotate.End, reduceBf.Curves.Rotate.End)
			}
		})
	}
}
