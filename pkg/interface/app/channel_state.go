...
type channelState struct {
	...
	selectedVertexesChannel     chan [][][]int // 選択頂点インデックス
	noSelectedVertexesChannel   chan [][][]int // 非選択頂点インデックス
...
func newChannelState() *channelState {
	return &channelState{
		...
		selectedVertexesChannel:     make(chan [][][]int, 1),
		noSelectedVertexesChannel:   make(chan [][][]int, 1),
...
func (channelState *channelState) SetSelectedVertexesChannel(v [][][]int) {
	channelState.selectedVertexesChannel <- v
}

func (channelState *channelState) SetNoSelectedVertexesChannel(v [][][]int) {
	channelState.noSelectedVertexesChannel <- v
}
