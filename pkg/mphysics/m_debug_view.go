...
type MDebugView struct {
	mbt.BtIDebugDraw
	defaultColors mbt.BtIDebugDraw_DefaultColors
	debugMode     int
	shader        *mgl.MShader
}
...
func NewMDebugView(shader *mgl.MShader) *MDebugView {
	return &MDebugView{
		defaultColors: NewMDefaultColors(),
		debugMode:     0,
		shader:        shader,
	}
}
...
func (mdv *MDebugView) Swigcptr() uintptr {
	...
	return uintptr(unsafe.Pointer(mdv))
}
...
func (mdv *MDebugView) DrawLine(a ...interface{}) {
	...
	if argc == 3 {
		...
		// 色を設定
		colorUniform := gl.GetUniformLocation(mdv.shader.PhysicsProgram, gl.Str(mgl.SHADER_PHYSICS_COLOR))
		gl.Uniform3f(colorUniform, color.GetX(), color.GetY(), color.GetZ())

		alphaUniform := gl.GetUniformLocation(mdv.shader.PhysicsProgram, gl.Str(mgl.SHADER_PHYSICS_ALPHA))
