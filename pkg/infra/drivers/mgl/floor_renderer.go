...
// NewFloorRenderer は新しい床レンダラーを作成する。
func NewFloorRenderer() *FloorRenderer {
	// 床のラインの頂点データ
	vertices := generateFloorVertices()

	factory := NewBufferFactory()
	bufferHandle := factory.NewFloorBuffer(vertices)
...
// Bind は床描画バッファをバインドする。
func (f *FloorRenderer) Bind() {
	f.bufferHandle.Bind()
}

// Unbind は床描画バッファをアンバインドする。
...
// generateFloorVertices は床描画用の頂点データを生成する。
