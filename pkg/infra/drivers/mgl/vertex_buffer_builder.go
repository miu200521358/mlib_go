...
// NewVertexBufferBuilder は頂点バッファビルダーを生成する。
...
func (b *VertexBufferBuilder) AddAttribute(attrType AttributeType) *VertexBufferBuilder {
	...
	attr := graphics_api.VertexAttribute{
...
// AddAttributeWithSize は頂点属性をカスタムサイズで追加する。
...
// AddStandardVertexAttributes は標準的な頂点属性を追加する。
...
// AddPositionColorAttributes は位置と色の属性を追加する。
...
// AddBoneAttributes はボーンの属性を追加する。
...
// AddOverrideAttributes はオーバーライドの属性を追加する。
...
func (b *VertexBufferBuilder) SetData(data unsafe.Pointer, arrayCount int) *VertexBufferBuilder {
	...
	// データのバッファリング
	b.vbo.BufferDataWithLayout(arrayCount, fieldCount, b.floatSize, data, graphics_api.BufferUsageStatic)
...
func (h *VertexBufferHandle) UpdateVertexDeltas(vertices *delta.VertexMorphDeltas) {
	...
	vertices.ForEach(func(vidx int, vertexDelta *delta.VertexMorphDelta) bool {
