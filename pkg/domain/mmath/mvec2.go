...
func (vec2 *MVec2) Inverted() *MVec2 {
	return &MVec2{-vec2.X, -vec2.Y}
}
...
func (vec2 *MVec2) Clamp(min, max *MVec2) *MVec2 {
	vec2.X = ClampedFloat(vec2.X, min.X, max.X)
	vec2.Y = ClampedFloat(vec2.Y, min.Y, max.Y)
	return vec2
}
