...
// MLIB_PREFIX はシステム用接頭辞。
const MLIB_PREFIX string = "[mlib]"

// BONE_DIRECTION_PREFIX は左右置換用の接頭辞。
const BONE_DIRECTION_PREFIX string = "{d}"

// BoneDirection はボーンの左右方向を表す。
...
// StringFromDirection は左右方向を反映した文字列を返す。
...
// StringFromDirectionAndIdx は左右方向とインデックスを反映した文字列を返す。
...
// GetStandardBoneConfigs は標準ボーン設定を返す。
