...
func skinningMatrix(deform model.IDeform, boneDeltas *delta.BoneDeltas) mmath.Mat4 {
	...
	for i, boneIndex := range indexes {
		...
		var weightedMat mmath.Mat4
		if boneDelta == nil {
			// 差分が無いボーンは単位行列として扱う。
			weightedMat = mmath.IDENT_MAT4
			weightedMat.MulScalar(weight)
			mat.Add(weightedMat)
			continue
		}
		weightedMat = boneDelta.FilledLocalMatrix()
		weightedMat.MulScalar(weight)
		mat.Add(weightedMat)
	}
	return mat
